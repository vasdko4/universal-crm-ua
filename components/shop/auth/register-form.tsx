...
export function RegisterForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!normPhone) return setError('Введите корректный номер телефона (например, +380 67 123 45 67)')
    ...
    if (!phoneCheck.available) {
      setLoading(false)
      return setError(phoneCheck.error ?? 'Этот номер телефона уже зарегистрирован')
    }
    ...
    if (error) {
      setLoading(false)
      if (/exist|already|unique/i.test(error.message ?? ''))
        return setError('Пользователь с такой почтой уже существует')
      return setError(error.message ?? 'Ошибка регистрации')
    }
