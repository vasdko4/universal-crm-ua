...
async function main() {
  try {
    ...
    console.log('\n✓ Database ready.')
    if (withSeed) {
      console.log('  Demo catalog loaded (no login accounts).')
      console.log('  Create an admin via the wizard at http://localhost:3000')
      console.log('  CI: node scripts/e2e-bootstrap.mjs with E2E_ADMIN_EMAIL / E2E_ADMIN_PASSWORD')
    } else {
