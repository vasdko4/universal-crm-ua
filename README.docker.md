...
После запуска скрипт печатает ссылку мастера с токеном (`/setup?token=…`). Откройте её — поле токена заполнять не нужно.
...
**Хотите вместо этого сразу получить демо-каталог** (товары, категории,
тестовые заказы)? Выполните ДО первого открытия сайта:
```bash
docker compose exec app node scripts/db-setup.mjs --seed
```
Это загрузит демо-данные. Администратора вы создадите через мастер установки
в браузере при первом заходе на сайт.
