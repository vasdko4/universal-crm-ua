...
| Мастер первой установки | `/setup?token=…` (токен — `SETUP_TOKEN` / `.setup-token`, печатается в логе при первом старте; после завершения закрывается) |
...
Скрипт печати готовую ссылку мастера (`/setup?token=…`) — токен уже в ней, поле заполнять не нужно. Если ссылки нет: `SETUP_TOKEN` в `.env` или лог контейнера (`[setup] one-time install token`). Откроется мастер:
